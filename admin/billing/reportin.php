...
<div class="table-responsive">
							<table class="table table-dark table-striped">
                                    <thead>
                                        <tr>
                                            <th><h5 style="color: red;"><B>INPATIENT BILLING REPORT-->

  <select name="" onchange="location = this.value;">
 <option>-PLS SELECT REPORT-</option>
...
<th>(INPATIENT) NUMBER OF BILLS</th>
<th>    <?php 
//$sql ="SELECT billingnos from  invoice_order ";

$sql = "SELECT invoice_order.billingnos from  invoice_order 
LEFT JOIN patientinfo on invoice_order.hospitalnos = patientinfo.hospitalnos
where patientinfo.ptype = 'IN-PATIENT' ";
...
<tbody style="background-color: red;color: white;"  >
    <tr>
    <th>(INPATIENT) NUMBER OF PATIENT</th>
    <th>
    <?php 
$sql ="SELECT hospitalnos from  patientinfo where ptype = 'IN-PATIENT'";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$bg=$query->rowCount();
...
<tbody>
<?php
$sql = "SELECT * from  invoice_order where ptype = 'IN-PATIENT'";
$query = $dbh->prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
<div class="float-left">
 <table  class="table table-dark table-striped">
  <thead>
    <tr> 
      <th h5 class="card-title text-LEFT" style="color:red;">(INPATIENT) TOTAL-BILL(N)</th>
      <th><?php echo $x;?></th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <th h5 class="card-title text-LEFT" style="color:red;">(INPATIENT) TOTAL-PAYMENT(N)</th>
      <td><?php echo $y;?></td>
    </tr>

    <tr>
      <th h5 class="card-title text-LEFT" style="color:red;">(INPATIENT) TOTAL-BALANCE(N)</th>
      <td><?php echo $z;?></td>
    </tr>
  </tbody>
</table>
